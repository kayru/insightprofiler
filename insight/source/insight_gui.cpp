...
namespace Insight
{
	...
	InsightGraphD3D9::InsightGraphD3D9()
		: m_d3d9(NULL)
		, m_device(NULL)
		, m_device_lost(false)
		, m_rect()
	...
	void InsightGraphD3D9::resize( RECT rect )
	{
		m_rect = rect;
	}

	void InsightGraphD3D9::init( HWND hwnd )
	{
		m_hwnd = hwnd;
	...
	void InsightGraphD3D9::draw_rect( float x1, float y1, float x2, float y2, DWORD colour )
	{
	...
	void InsightGraphD3D9::zoom( float delta_z )
	{
	...
	bool InsightGraphD3D9::add_bar( size_t thread_idx, size_t depth, const Token& t )
	{
	...
	void InsightGraphD3D9::drag( long x, long y )
	{
	...
	void InsightGraphD3D9::select( long a, long b )
	{		
		float unused;
	...
	void InsightGraphD3D9::update_viewport()
	{
		D3DVIEWPORT9 vp;
	...
	void InsightGraphD3D9::set_cursor( long cx, long cy )
	{				
	...
	InsightGraphD3D9::BarInfo* InsightGraphD3D9::selected()
	{
		return m_selected;
	}

	bool InsightGraphD3D9::handle_device_lost()
	{		
		if( m_device_lost )
		{
	...
	void InsightGraphD3D9::set_render_states()
	{
	...
	void InsightGraphD3D9::draw_temporary_objects()
	{
		m_tmp_vertices.reset();
		if( m_selected )
		{
	...
	void InsightGraphD3D9::client_to_time( long cx, long cy, float& tx, float& ty )
	{
		float w = float(m_rect.right-m_rect.left);
		float h = float(m_rect.bottom-m_rect.top);
	...
	float InsightGraphD3D9::selection_length()
	{
